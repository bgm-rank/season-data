...
从 MyAnimeList 获取季度新番数据，保存到 `release/mal/{year}-{season}.json`。
...
# 批量获取（自动跳过已有文件）
PYTHONPATH=src uv run python -m services.mal range 2000 winter 2026 spring
```

需要在 `.env` 中配置 `MAL_CLIENT_ID`。
